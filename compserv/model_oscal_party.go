...
type OscalParty struct {
	...
	Party AssemblyOscalMetadataParty `json:"party,omitempty"`
}

// AssertOscalPartyRequired checks if the required fields are not zero-ed
func AssertOscalPartyRequired(obj OscalParty) error {
	if err := AssertAssemblyOscalMetadataPartyRequired(obj.Party); err != nil {
		return err
	}
	return nil
}

// AssertRecurseOscalPartyRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of OscalParty (e.g. [][]OscalParty), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseOscalPartyRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aOscalParty, ok := obj.(OscalParty)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertOscalPartyRequired(aOscalParty)
	})
}
