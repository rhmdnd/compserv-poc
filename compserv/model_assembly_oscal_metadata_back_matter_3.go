...
// AssemblyOscalMetadataBackMatter3 - A collection of resources, which may be included directly or by reference.
type AssemblyOscalMetadataBackMatter3 struct {

	Resources []Resource3 `json:"resources,omitempty"`
}

// AssertAssemblyOscalMetadataBackMatter3Required checks if the required fields are not zero-ed
func AssertAssemblyOscalMetadataBackMatter3Required(obj AssemblyOscalMetadataBackMatter3) error {
	for _, el := range obj.Resources {
		if err := AssertResource3Required(el); err != nil {
			return err
		}
	}
	return nil
...
// AssertRecurseAssemblyOscalMetadataBackMatter3Required recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of AssemblyOscalMetadataBackMatter3 (e.g. [][]AssemblyOscalMetadataBackMatter3), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseAssemblyOscalMetadataBackMatter3Required(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aAssemblyOscalMetadataBackMatter3, ok := obj.(AssemblyOscalMetadataBackMatter3)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertAssemblyOscalMetadataBackMatter3Required(aAssemblyOscalMetadataBackMatter3)
	})
}
