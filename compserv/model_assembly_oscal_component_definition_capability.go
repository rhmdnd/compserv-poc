...
// AssemblyOscalComponentDefinitionCapability - A grouping of other components and/or capabilities.
type AssemblyOscalComponentDefinitionCapability struct {
	...
	IncorporatesComponents []AssemblyOscalComponentDefinitionIncorporatesComponent `json:"incorporates-components,omitempty"`
...
// AssertAssemblyOscalComponentDefinitionCapabilityRequired checks if the required fields are not zero-ed
func AssertAssemblyOscalComponentDefinitionCapabilityRequired(obj AssemblyOscalComponentDefinitionCapability) error {
	...
	for _, el := range obj.IncorporatesComponents {
		if err := AssertAssemblyOscalComponentDefinitionIncorporatesComponentRequired(el); err != nil {
			return err
		}
	}
...
// AssertRecurseAssemblyOscalComponentDefinitionCapabilityRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of AssemblyOscalComponentDefinitionCapability (e.g. [][]AssemblyOscalComponentDefinitionCapability), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseAssemblyOscalComponentDefinitionCapabilityRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aAssemblyOscalComponentDefinitionCapability, ok := obj.(AssemblyOscalComponentDefinitionCapability)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertAssemblyOscalComponentDefinitionCapabilityRequired(aAssemblyOscalComponentDefinitionCapability)
	})
}
