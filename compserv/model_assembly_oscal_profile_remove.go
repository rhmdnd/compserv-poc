...
// AssemblyOscalProfileRemove - Specifies objects to be removed from a control based on specific aspects of the object that must all match.
type AssemblyOscalProfileRemove struct {
...
// AssertAssemblyOscalProfileRemoveRequired checks if the required fields are not zero-ed
func AssertAssemblyOscalProfileRemoveRequired(obj AssemblyOscalProfileRemove) error {
	return nil
}

// AssertRecurseAssemblyOscalProfileRemoveRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of AssemblyOscalProfileRemove (e.g. [][]AssemblyOscalProfileRemove), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseAssemblyOscalProfileRemoveRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aAssemblyOscalProfileRemove, ok := obj.(AssemblyOscalProfileRemove)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertAssemblyOscalProfileRemoveRequired(aAssemblyOscalProfileRemove)
	})
}
