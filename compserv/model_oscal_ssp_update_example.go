...
type OscalSspUpdateExample struct {

	SystemSecurityPlan OscalPartialUpdateExample `json:"system-security-plan,omitempty"`
}

// AssertOscalSspUpdateExampleRequired checks if the required fields are not zero-ed
func AssertOscalSspUpdateExampleRequired(obj OscalSspUpdateExample) error {
	if err := AssertOscalPartialUpdateExampleRequired(obj.SystemSecurityPlan); err != nil {
		return err
	}
	return nil
}

// AssertRecurseOscalSspUpdateExampleRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of OscalSspUpdateExample (e.g. [][]OscalSspUpdateExample), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseOscalSspUpdateExampleRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aOscalSspUpdateExample, ok := obj.(OscalSspUpdateExample)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertOscalSspUpdateExampleRequired(aOscalSspUpdateExample)
	})
}
