...
// AssemblyOscalCatalogCatalog - A collection of controls.
type AssemblyOscalCatalogCatalog struct {

	// A globally unique identifier for this catalog instance. This UUID should be changed when this document is revised.
	Uuid string `json:"uuid"`

	Metadata AssemblyOscalMetadataMetadata `json:"metadata"`
	...
	Groups []AssemblyOscalCatalogGroup `json:"groups,omitempty"`

	BackMatter AssemblyOscalMetadataBackMatter `json:"back-matter,omitempty"`
}

// AssertAssemblyOscalCatalogCatalogRequired checks if the required fields are not zero-ed
func AssertAssemblyOscalCatalogCatalogRequired(obj AssemblyOscalCatalogCatalog) error {
	...
	if err := AssertAssemblyOscalMetadataMetadataRequired(obj.Metadata); err != nil {
		return err
	}
	...
	for _, el := range obj.Groups {
		if err := AssertAssemblyOscalCatalogGroupRequired(el); err != nil {
			return err
		}
	}
	if err := AssertAssemblyOscalMetadataBackMatterRequired(obj.BackMatter); err != nil {
		return err
	}
	return nil
}

// AssertRecurseAssemblyOscalCatalogCatalogRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of AssemblyOscalCatalogCatalog (e.g. [][]AssemblyOscalCatalogCatalog), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseAssemblyOscalCatalogCatalogRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aAssemblyOscalCatalogCatalog, ok := obj.(AssemblyOscalCatalogCatalog)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertAssemblyOscalCatalogCatalogRequired(aAssemblyOscalCatalogCatalog)
	})
}
