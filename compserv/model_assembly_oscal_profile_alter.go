...
// AssemblyOscalProfileAlter - An Alter element specifies changes to be made to an included control when a profile is resolved.
type AssemblyOscalProfileAlter struct {

	// A reference to a control with a corresponding id value.
	ControlId string `json:"control-id,omitempty"`

	Removes []AssemblyOscalProfileRemove `json:"removes,omitempty"`

	Adds []AssemblyOscalProfileAdd `json:"adds,omitempty"`
}

// AssertAssemblyOscalProfileAlterRequired checks if the required fields are not zero-ed
func AssertAssemblyOscalProfileAlterRequired(obj AssemblyOscalProfileAlter) error {
	for _, el := range obj.Removes {
		if err := AssertAssemblyOscalProfileRemoveRequired(el); err != nil {
			return err
		}
	}
	for _, el := range obj.Adds {
		if err := AssertAssemblyOscalProfileAddRequired(el); err != nil {
			return err
		}
	}
	return nil
...
// AssertRecurseAssemblyOscalProfileAlterRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of AssemblyOscalProfileAlter (e.g. [][]AssemblyOscalProfileAlter), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseAssemblyOscalProfileAlterRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aAssemblyOscalProfileAlter, ok := obj.(AssemblyOscalProfileAlter)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertAssemblyOscalProfileAlterRequired(aAssemblyOscalProfileAlter)
	})
}
