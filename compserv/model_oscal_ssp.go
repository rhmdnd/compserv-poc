...
type OscalSsp struct {

	SystemSecurityPlan AssemblyOscalSspSystemSecurityPlan `json:"system-security-plan,omitempty"`
}

// AssertOscalSspRequired checks if the required fields are not zero-ed
func AssertOscalSspRequired(obj OscalSsp) error {
	if err := AssertAssemblyOscalSspSystemSecurityPlanRequired(obj.SystemSecurityPlan); err != nil {
		return err
	}
	return nil
}

// AssertRecurseOscalSspRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of OscalSsp (e.g. [][]OscalSsp), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseOscalSspRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aOscalSsp, ok := obj.(OscalSsp)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertOscalSspRequired(aOscalSsp)
	})
}
