...
// AssertAssemblyOscalMetadataMetadata1Required checks if the required fields are not zero-ed
func AssertAssemblyOscalMetadataMetadata1Required(obj AssemblyOscalMetadataMetadata1) error {
	...
	for _, el := range obj.Revisions {
		if err := AssertAssemblyOscalMetadataRevision1Required(el); err != nil {
			return err
		}
	}
	...
	for _, el := range obj.Roles {
		if err := AssertAssemblyOscalMetadataRole1Required(el); err != nil {
			return err
		}
	}
	for _, el := range obj.Locations {
		if err := AssertAssemblyOscalMetadataLocation1Required(el); err != nil {
			return err
		}
	}
	for _, el := range obj.Parties {
		if err := AssertAssemblyOscalMetadataParty1Required(el); err != nil {
			return err
		}
	}
	for _, el := range obj.ResponsibleParties {
		if err := AssertAssemblyOscalMetadataResponsibleParty1Required(el); err != nil {
			return err
		}
	}
	return nil
...
// AssertRecurseAssemblyOscalMetadataMetadata1Required recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of AssemblyOscalMetadataMetadata1 (e.g. [][]AssemblyOscalMetadataMetadata1), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseAssemblyOscalMetadataMetadata1Required(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aAssemblyOscalMetadataMetadata1, ok := obj.(AssemblyOscalMetadataMetadata1)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertAssemblyOscalMetadataMetadata1Required(aAssemblyOscalMetadataMetadata1)
	})
}
