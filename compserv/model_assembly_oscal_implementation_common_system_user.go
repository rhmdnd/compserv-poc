...
// AssemblyOscalImplementationCommonSystemUser - A type of user that interacts with the system based on an associated role.
type AssemblyOscalImplementationCommonSystemUser struct {
	...
	AuthorizedPrivileges []AssemblyOscalImplementationCommonAuthorizedPrivilege `json:"authorized-privileges,omitempty"`

	// Additional commentary on the containing object.
	Remarks string `json:"remarks,omitempty"`
}

// AssertAssemblyOscalImplementationCommonSystemUserRequired checks if the required fields are not zero-ed
func AssertAssemblyOscalImplementationCommonSystemUserRequired(obj AssemblyOscalImplementationCommonSystemUser) error {
	...
	for _, el := range obj.AuthorizedPrivileges {
		if err := AssertAssemblyOscalImplementationCommonAuthorizedPrivilegeRequired(el); err != nil {
			return err
		}
	}
	return nil
...
// AssertRecurseAssemblyOscalImplementationCommonSystemUserRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of AssemblyOscalImplementationCommonSystemUser (e.g. [][]AssemblyOscalImplementationCommonSystemUser), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseAssemblyOscalImplementationCommonSystemUserRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aAssemblyOscalImplementationCommonSystemUser, ok := obj.(AssemblyOscalImplementationCommonSystemUser)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertAssemblyOscalImplementationCommonSystemUserRequired(aAssemblyOscalImplementationCommonSystemUser)
	})
}
