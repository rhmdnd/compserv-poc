...
// AssemblyOscalMetadataRevision3 - An entry in a sequential list of revisions to the containing document in reverse chronological order (i.e., most recent previous revision first).
type AssemblyOscalMetadataRevision3 struct {
...
// AssertAssemblyOscalMetadataRevision3Required checks if the required fields are not zero-ed
func AssertAssemblyOscalMetadataRevision3Required(obj AssemblyOscalMetadataRevision3) error {
...
// AssertRecurseAssemblyOscalMetadataRevision3Required recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of AssemblyOscalMetadataRevision3 (e.g. [][]AssemblyOscalMetadataRevision3), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseAssemblyOscalMetadataRevision3Required(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aAssemblyOscalMetadataRevision3, ok := obj.(AssemblyOscalMetadataRevision3)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertAssemblyOscalMetadataRevision3Required(aAssemblyOscalMetadataRevision3)
	})
}
