...
// AssemblyOscalProfileProfile - Each OSCAL profile is defined by a Profile element
type AssemblyOscalProfileProfile struct {

	// A globally unique identifier for this profile instance. This UUID should be changed when this document is revised.
	Uuid string `json:"uuid"`

	Metadata AssemblyOscalMetadataMetadata1 `json:"metadata"`

	Imports []AssemblyOscalProfileImport `json:"imports"`

	Merge AssemblyOscalProfileMerge `json:"merge,omitempty"`

	Modify AssemblyOscalProfileModify `json:"modify,omitempty"`

	BackMatter AssemblyOscalMetadataBackMatter1 `json:"back-matter,omitempty"`
}

// AssertAssemblyOscalProfileProfileRequired checks if the required fields are not zero-ed
func AssertAssemblyOscalProfileProfileRequired(obj AssemblyOscalProfileProfile) error {
	...
	if err := AssertAssemblyOscalMetadataMetadata1Required(obj.Metadata); err != nil {
		return err
	}
	for _, el := range obj.Imports {
		if err := AssertAssemblyOscalProfileImportRequired(el); err != nil {
			return err
		}
	}
	if err := AssertAssemblyOscalProfileMergeRequired(obj.Merge); err != nil {
		return err
	}
	if err := AssertAssemblyOscalProfileModifyRequired(obj.Modify); err != nil {
		return err
	}
	if err := AssertAssemblyOscalMetadataBackMatter1Required(obj.BackMatter); err != nil {
		return err
	}
	return nil
}

// AssertRecurseAssemblyOscalProfileProfileRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of AssemblyOscalProfileProfile (e.g. [][]AssemblyOscalProfileProfile), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseAssemblyOscalProfileProfileRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aAssemblyOscalProfileProfile, ok := obj.(AssemblyOscalProfileProfile)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertAssemblyOscalProfileProfileRequired(aAssemblyOscalProfileProfile)
	})
}
