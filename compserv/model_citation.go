...
// AssertCitationRequired checks if the required fields are not zero-ed
func AssertCitationRequired(obj Citation) error {
...
// AssertRecurseCitationRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of Citation (e.g. [][]Citation), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseCitationRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aCitation, ok := obj.(Citation)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertCitationRequired(aCitation)
	})
}
