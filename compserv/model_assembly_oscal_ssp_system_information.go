...
// AssemblyOscalSspSystemInformation - Contains details about all information types that are stored, processed, or transmitted by the system, such as privacy information, and those defined in NIST SP 800-60.
type AssemblyOscalSspSystemInformation struct {
...
// AssertAssemblyOscalSspSystemInformationRequired checks if the required fields are not zero-ed
func AssertAssemblyOscalSspSystemInformationRequired(obj AssemblyOscalSspSystemInformation) error {
	elements := map[string]interface{}{
		"information-types": obj.InformationTypes,
	}
	...
	for _, el := range obj.InformationTypes {
		if err := AssertInformationTypeRequired(el); err != nil {
			return err
		}
	}
	return nil
...
// AssertRecurseAssemblyOscalSspSystemInformationRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of AssemblyOscalSspSystemInformation (e.g. [][]AssemblyOscalSspSystemInformation), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseAssemblyOscalSspSystemInformationRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aAssemblyOscalSspSystemInformation, ok := obj.(AssemblyOscalSspSystemInformation)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertAssemblyOscalSspSystemInformationRequired(aAssemblyOscalSspSystemInformation)
	})
}
