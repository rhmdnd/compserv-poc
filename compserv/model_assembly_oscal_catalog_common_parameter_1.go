...
// AssemblyOscalCatalogCommonParameter1 - Parameters provide a mechanism for the dynamic assignment of value(s) in a control.
type AssemblyOscalCatalogCommonParameter1 struct {
...
// AssertAssemblyOscalCatalogCommonParameter1Required checks if the required fields are not zero-ed
func AssertAssemblyOscalCatalogCommonParameter1Required(obj AssemblyOscalCatalogCommonParameter1) error {
	...
	for _, el := range obj.Constraints {
		if err := AssertAssemblyOscalCatalogCommonParameterConstraint1Required(el); err != nil {
			return err
		}
	}
...
// AssertRecurseAssemblyOscalCatalogCommonParameter1Required recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of AssemblyOscalCatalogCommonParameter1 (e.g. [][]AssemblyOscalCatalogCommonParameter1), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseAssemblyOscalCatalogCommonParameter1Required(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aAssemblyOscalCatalogCommonParameter1, ok := obj.(AssemblyOscalCatalogCommonParameter1)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertAssemblyOscalCatalogCommonParameter1Required(aAssemblyOscalCatalogCommonParameter1)
	})
}
