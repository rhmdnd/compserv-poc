...
// AssemblyOscalProfileCombine - A Combine element defines whether and how to combine multiple (competing) versions of the same control
type AssemblyOscalProfileCombine struct {

	// How clashing controls should be handled
	Method string `json:"method,omitempty"`
}

// AssertAssemblyOscalProfileCombineRequired checks if the required fields are not zero-ed
func AssertAssemblyOscalProfileCombineRequired(obj AssemblyOscalProfileCombine) error {
	return nil
}

// AssertRecurseAssemblyOscalProfileCombineRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of AssemblyOscalProfileCombine (e.g. [][]AssemblyOscalProfileCombine), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseAssemblyOscalProfileCombineRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aAssemblyOscalProfileCombine, ok := obj.(AssemblyOscalProfileCombine)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertAssemblyOscalProfileCombineRequired(aAssemblyOscalProfileCombine)
	})
}
