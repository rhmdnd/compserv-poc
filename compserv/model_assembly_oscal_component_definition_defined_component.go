...
// AssemblyOscalComponentDefinitionDefinedComponent - A defined component that can be part of an implemented system.
type AssemblyOscalComponentDefinitionDefinedComponent struct {
	...
	Protocols []AssemblyOscalImplementationCommonProtocol `json:"protocols,omitempty"`

	ControlImplementations []AssemblyOscalComponentDefinitionControlImplementation `json:"control-implementations,omitempty"`

	// Additional commentary on the containing object.
	Remarks string `json:"remarks,omitempty"`
}

// AssertAssemblyOscalComponentDefinitionDefinedComponentRequired checks if the required fields are not zero-ed
func AssertAssemblyOscalComponentDefinitionDefinedComponentRequired(obj AssemblyOscalComponentDefinitionDefinedComponent) error {
	...
	for _, el := range obj.Protocols {
		if err := AssertAssemblyOscalImplementationCommonProtocolRequired(el); err != nil {
			return err
		}
	}
	for _, el := range obj.ControlImplementations {
		if err := AssertAssemblyOscalComponentDefinitionControlImplementationRequired(el); err != nil {
			return err
		}
	}
	return nil
...
// AssertRecurseAssemblyOscalComponentDefinitionDefinedComponentRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of AssemblyOscalComponentDefinitionDefinedComponent (e.g. [][]AssemblyOscalComponentDefinitionDefinedComponent), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseAssemblyOscalComponentDefinitionDefinedComponentRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aAssemblyOscalComponentDefinitionDefinedComponent, ok := obj.(AssemblyOscalComponentDefinitionDefinedComponent)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertAssemblyOscalComponentDefinitionDefinedComponentRequired(aAssemblyOscalComponentDefinitionDefinedComponent)
	})
}
