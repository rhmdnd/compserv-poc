...
type OscalCatalogUpdateExample struct {

	Catalog OscalPartialUpdateExample `json:"catalog,omitempty"`
}

// AssertOscalCatalogUpdateExampleRequired checks if the required fields are not zero-ed
func AssertOscalCatalogUpdateExampleRequired(obj OscalCatalogUpdateExample) error {
	if err := AssertOscalPartialUpdateExampleRequired(obj.Catalog); err != nil {
		return err
	}
	return nil
}

// AssertRecurseOscalCatalogUpdateExampleRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of OscalCatalogUpdateExample (e.g. [][]OscalCatalogUpdateExample), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseOscalCatalogUpdateExampleRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aOscalCatalogUpdateExample, ok := obj.(OscalCatalogUpdateExample)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertOscalCatalogUpdateExampleRequired(aOscalCatalogUpdateExample)
	})
}
