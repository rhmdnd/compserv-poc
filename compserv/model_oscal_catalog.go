...
type OscalCatalog struct {
	...
	Catalog AssemblyOscalCatalogCatalog `json:"catalog,omitempty"`
}

// AssertOscalCatalogRequired checks if the required fields are not zero-ed
func AssertOscalCatalogRequired(obj OscalCatalog) error {
	if err := AssertAssemblyOscalCatalogCatalogRequired(obj.Catalog); err != nil {
		return err
	}
	return nil
}

// AssertRecurseOscalCatalogRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of OscalCatalog (e.g. [][]OscalCatalog), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseOscalCatalogRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aOscalCatalog, ok := obj.(OscalCatalog)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertOscalCatalogRequired(aOscalCatalog)
	})
}
