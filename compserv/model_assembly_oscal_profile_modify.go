...
// AssemblyOscalProfileModify - Set parameters or amend controls in resolution
type AssemblyOscalProfileModify struct {

	SetParameters []ParameterSetting `json:"set-parameters,omitempty"`

	Alters []AssemblyOscalProfileAlter `json:"alters,omitempty"`
}

// AssertAssemblyOscalProfileModifyRequired checks if the required fields are not zero-ed
func AssertAssemblyOscalProfileModifyRequired(obj AssemblyOscalProfileModify) error {
	for _, el := range obj.SetParameters {
		if err := AssertParameterSettingRequired(el); err != nil {
			return err
		}
	}
	for _, el := range obj.Alters {
		if err := AssertAssemblyOscalProfileAlterRequired(el); err != nil {
			return err
		}
	}
	return nil
...
// AssertRecurseAssemblyOscalProfileModifyRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of AssemblyOscalProfileModify (e.g. [][]AssemblyOscalProfileModify), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseAssemblyOscalProfileModifyRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aAssemblyOscalProfileModify, ok := obj.(AssemblyOscalProfileModify)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertAssemblyOscalProfileModifyRequired(aAssemblyOscalProfileModify)
	})
}
