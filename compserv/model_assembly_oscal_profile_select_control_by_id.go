...
// AssemblyOscalProfileSelectControlById - Call a control by its ID
type AssemblyOscalProfileSelectControlById struct {

	// When a control is included, whether its child (dependent) controls are also included.
	WithChildControls string `json:"with-child-controls,omitempty"`
...
// AssertAssemblyOscalProfileSelectControlByIdRequired checks if the required fields are not zero-ed
func AssertAssemblyOscalProfileSelectControlByIdRequired(obj AssemblyOscalProfileSelectControlById) error {
	for _, el := range obj.Matching {
		if err := AssertMatchControlsByPatternRequired(el); err != nil {
			return err
		}
	}
	return nil
...
// AssertRecurseAssemblyOscalProfileSelectControlByIdRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of AssemblyOscalProfileSelectControlById (e.g. [][]AssemblyOscalProfileSelectControlById), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseAssemblyOscalProfileSelectControlByIdRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aAssemblyOscalProfileSelectControlById, ok := obj.(AssemblyOscalProfileSelectControlById)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertAssemblyOscalProfileSelectControlByIdRequired(aAssemblyOscalProfileSelectControlById)
	})
}
