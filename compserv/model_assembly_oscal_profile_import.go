...
// AssemblyOscalProfileImport - The import designates a catalog, profile, or other resource to be included (referenced and potentially modified) by this profile. The import also identifies which controls to select using the include-all, include-controls, and exclude-controls directives.
type AssemblyOscalProfileImport struct {
	...
	IncludeAll map[string]interface{} `json:"include-all,omitempty"`

	IncludeControls []AssemblyOscalProfileSelectControlById `json:"include-controls,omitempty"`

	ExcludeControls []AssemblyOscalProfileSelectControlById `json:"exclude-controls,omitempty"`
}

// AssertAssemblyOscalProfileImportRequired checks if the required fields are not zero-ed
func AssertAssemblyOscalProfileImportRequired(obj AssemblyOscalProfileImport) error {
	...
	for _, el := range obj.IncludeControls {
		if err := AssertAssemblyOscalProfileSelectControlByIdRequired(el); err != nil {
			return err
		}
	}
	for _, el := range obj.ExcludeControls {
		if err := AssertAssemblyOscalProfileSelectControlByIdRequired(el); err != nil {
			return err
		}
	}
...
// AssertRecurseAssemblyOscalProfileImportRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of AssemblyOscalProfileImport (e.g. [][]AssemblyOscalProfileImport), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseAssemblyOscalProfileImportRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aAssemblyOscalProfileImport, ok := obj.(AssemblyOscalProfileImport)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertAssemblyOscalProfileImportRequired(aAssemblyOscalProfileImport)
	})
}
