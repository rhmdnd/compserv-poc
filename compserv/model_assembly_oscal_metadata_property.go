...
// AssemblyOscalMetadataProperty - An attribute, characteristic, or quality of the containing object expressed as a namespace qualified name/value pair. The value of a property is a simple scalar value, which may be expressed as a list of values.
type AssemblyOscalMetadataProperty struct {
...
// AssertAssemblyOscalMetadataPropertyRequired checks if the required fields are not zero-ed
func AssertAssemblyOscalMetadataPropertyRequired(obj AssemblyOscalMetadataProperty) error {
...
// AssertRecurseAssemblyOscalMetadataPropertyRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of AssemblyOscalMetadataProperty (e.g. [][]AssemblyOscalMetadataProperty), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseAssemblyOscalMetadataPropertyRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aAssemblyOscalMetadataProperty, ok := obj.(AssemblyOscalMetadataProperty)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertAssemblyOscalMetadataPropertyRequired(aAssemblyOscalMetadataProperty)
	})
}
