...
// AssemblyOscalSspSystemImplementation - Provides information as to how the system is implemented.
type AssemblyOscalSspSystemImplementation struct {
	...
	Users []AssemblyOscalImplementationCommonSystemUser `json:"users"`

	Components []AssemblyOscalImplementationCommonSystemComponent `json:"components"`

	InventoryItems []AssemblyOscalImplementationCommonInventoryItem `json:"inventory-items,omitempty"`

	// Additional commentary on the containing object.
	Remarks string `json:"remarks,omitempty"`
}

// AssertAssemblyOscalSspSystemImplementationRequired checks if the required fields are not zero-ed
func AssertAssemblyOscalSspSystemImplementationRequired(obj AssemblyOscalSspSystemImplementation) error {
	...
	for _, el := range obj.LeveragedAuthorizations {
		if err := AssertLeveragedAuthorizationRequired(el); err != nil {
			return err
		}
	}
	for _, el := range obj.Users {
		if err := AssertAssemblyOscalImplementationCommonSystemUserRequired(el); err != nil {
			return err
		}
	}
	for _, el := range obj.Components {
		if err := AssertAssemblyOscalImplementationCommonSystemComponentRequired(el); err != nil {
			return err
		}
	}
	for _, el := range obj.InventoryItems {
		if err := AssertAssemblyOscalImplementationCommonInventoryItemRequired(el); err != nil {
			return err
		}
	}
	return nil
...
// AssertRecurseAssemblyOscalSspSystemImplementationRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of AssemblyOscalSspSystemImplementation (e.g. [][]AssemblyOscalSspSystemImplementation), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseAssemblyOscalSspSystemImplementationRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aAssemblyOscalSspSystemImplementation, ok := obj.(AssemblyOscalSspSystemImplementation)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertAssemblyOscalSspSystemImplementationRequired(aAssemblyOscalSspSystemImplementation)
	})
}
