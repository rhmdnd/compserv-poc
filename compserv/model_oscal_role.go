...
type OscalRole struct {
	...
	Role AssemblyOscalMetadataRole `json:"role,omitempty"`
}

// AssertOscalRoleRequired checks if the required fields are not zero-ed
func AssertOscalRoleRequired(obj OscalRole) error {
	if err := AssertAssemblyOscalMetadataRoleRequired(obj.Role); err != nil {
		return err
	}
	return nil
}

// AssertRecurseOscalRoleRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of OscalRole (e.g. [][]OscalRole), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseOscalRoleRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aOscalRole, ok := obj.(OscalRole)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertOscalRoleRequired(aOscalRole)
	})
}
