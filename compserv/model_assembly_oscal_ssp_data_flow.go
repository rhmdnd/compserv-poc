...
// AssemblyOscalSspDataFlow - A description of the logical flow of information within the system and across its boundaries, optionally supplemented by diagrams that illustrate these flows.
type AssemblyOscalSspDataFlow struct {
...
// AssertAssemblyOscalSspDataFlowRequired checks if the required fields are not zero-ed
func AssertAssemblyOscalSspDataFlowRequired(obj AssemblyOscalSspDataFlow) error {
...
// AssertRecurseAssemblyOscalSspDataFlowRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of AssemblyOscalSspDataFlow (e.g. [][]AssemblyOscalSspDataFlow), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseAssemblyOscalSspDataFlowRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aAssemblyOscalSspDataFlow, ok := obj.(AssemblyOscalSspDataFlow)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertAssemblyOscalSspDataFlowRequired(aAssemblyOscalSspDataFlow)
	})
}
