...
type OscalComponentDefinition struct {
	...
	ComponentDefinition AssemblyOscalComponentDefinitionComponentDefinition `json:"component-definition,omitempty"`
}

// AssertOscalComponentDefinitionRequired checks if the required fields are not zero-ed
func AssertOscalComponentDefinitionRequired(obj OscalComponentDefinition) error {
	if err := AssertAssemblyOscalComponentDefinitionComponentDefinitionRequired(obj.ComponentDefinition); err != nil {
		return err
	}
	return nil
}

// AssertRecurseOscalComponentDefinitionRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of OscalComponentDefinition (e.g. [][]OscalComponentDefinition), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseOscalComponentDefinitionRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aOscalComponentDefinition, ok := obj.(OscalComponentDefinition)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertOscalComponentDefinitionRequired(aOscalComponentDefinition)
	})
}
