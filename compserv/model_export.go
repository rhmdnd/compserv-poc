...
func AssertExportRequired(obj Export) error {
	...
	for _, el := range obj.Provided {
		if err := AssertProvidedControlImplementationRequired(el); err != nil {
			return err
		}
	}
	for _, el := range obj.Responsibilities {
		if err := AssertControlImplementationResponsibilityRequired(el); err != nil {
			return err
		}
	}
	return nil
...
// AssertRecurseExportRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of Export (e.g. [][]Export), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseExportRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aExport, ok := obj.(Export)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertExportRequired(aExport)
	})
}
