...
type OscalProfile struct {
	...
	Profile AssemblyOscalProfileProfile `json:"profile,omitempty"`
}

// AssertOscalProfileRequired checks if the required fields are not zero-ed
func AssertOscalProfileRequired(obj OscalProfile) error {
	if err := AssertAssemblyOscalProfileProfileRequired(obj.Profile); err != nil {
		return err
	}
	return nil
}

// AssertRecurseOscalProfileRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of OscalProfile (e.g. [][]OscalProfile), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseOscalProfileRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aOscalProfile, ok := obj.(OscalProfile)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertOscalProfileRequired(aOscalProfile)
	})
}
