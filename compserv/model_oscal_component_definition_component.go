...
type OscalComponentDefinitionComponent struct {

	Component AssemblyOscalComponentDefinitionDefinedComponent `json:"component,omitempty"`
}

// AssertOscalComponentDefinitionComponentRequired checks if the required fields are not zero-ed
func AssertOscalComponentDefinitionComponentRequired(obj OscalComponentDefinitionComponent) error {
	if err := AssertAssemblyOscalComponentDefinitionDefinedComponentRequired(obj.Component); err != nil {
		return err
	}
	return nil
}

// AssertRecurseOscalComponentDefinitionComponentRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of OscalComponentDefinitionComponent (e.g. [][]OscalComponentDefinitionComponent), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseOscalComponentDefinitionComponentRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aOscalComponentDefinitionComponent, ok := obj.(OscalComponentDefinitionComponent)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertOscalComponentDefinitionComponentRequired(aOscalComponentDefinitionComponent)
	})
}
