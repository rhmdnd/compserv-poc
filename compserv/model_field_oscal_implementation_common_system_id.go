...
// FieldOscalImplementationCommonSystemId - A unique identifier for the system described by this system security plan.
type FieldOscalImplementationCommonSystemId struct {
...
// AssertFieldOscalImplementationCommonSystemIdRequired checks if the required fields are not zero-ed
func AssertFieldOscalImplementationCommonSystemIdRequired(obj FieldOscalImplementationCommonSystemId) error {
...
// AssertRecurseFieldOscalImplementationCommonSystemIdRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of FieldOscalImplementationCommonSystemId (e.g. [][]FieldOscalImplementationCommonSystemId), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseFieldOscalImplementationCommonSystemIdRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aFieldOscalImplementationCommonSystemId, ok := obj.(FieldOscalImplementationCommonSystemId)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertFieldOscalImplementationCommonSystemIdRequired(aFieldOscalImplementationCommonSystemId)
	})
}
