...
// ConfidentialityImpactLevel - The expected level of impact resulting from the unauthorized disclosure of the described information.
...
// AssertConfidentialityImpactLevelRequired checks if the required fields are not zero-ed
func AssertConfidentialityImpactLevelRequired(obj ConfidentialityImpactLevel) error {
...
// AssertRecurseConfidentialityImpactLevelRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of ConfidentialityImpactLevel (e.g. [][]ConfidentialityImpactLevel), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseConfidentialityImpactLevelRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aConfidentialityImpactLevel, ok := obj.(ConfidentialityImpactLevel)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertConfidentialityImpactLevelRequired(aConfidentialityImpactLevel)
	})
}
