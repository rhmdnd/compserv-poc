...
// AssemblyOscalMetadataProperty1 - An attribute, characteristic, or quality of the containing object expressed as a namespace qualified name/value pair. The value of a property is a simple scalar value, which may be expressed as a list of values.
type AssemblyOscalMetadataProperty1 struct {
...
// AssertAssemblyOscalMetadataProperty1Required checks if the required fields are not zero-ed
func AssertAssemblyOscalMetadataProperty1Required(obj AssemblyOscalMetadataProperty1) error {
...
// AssertRecurseAssemblyOscalMetadataProperty1Required recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of AssemblyOscalMetadataProperty1 (e.g. [][]AssemblyOscalMetadataProperty1), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseAssemblyOscalMetadataProperty1Required(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aAssemblyOscalMetadataProperty1, ok := obj.(AssemblyOscalMetadataProperty1)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertAssemblyOscalMetadataProperty1Required(aAssemblyOscalMetadataProperty1)
	})
}
