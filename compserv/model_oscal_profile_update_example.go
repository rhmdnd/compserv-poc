...
type OscalProfileUpdateExample struct {

	Profile OscalPartialUpdateExample `json:"profile,omitempty"`
}

// AssertOscalProfileUpdateExampleRequired checks if the required fields are not zero-ed
func AssertOscalProfileUpdateExampleRequired(obj OscalProfileUpdateExample) error {
	if err := AssertOscalPartialUpdateExampleRequired(obj.Profile); err != nil {
		return err
	}
	return nil
}

// AssertRecurseOscalProfileUpdateExampleRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of OscalProfileUpdateExample (e.g. [][]OscalProfileUpdateExample), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseOscalProfileUpdateExampleRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aOscalProfileUpdateExample, ok := obj.(OscalProfileUpdateExample)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertOscalProfileUpdateExampleRequired(aOscalProfileUpdateExample)
	})
}
