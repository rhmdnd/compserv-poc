...
type OscalImplementedRequirement struct {
	...
	ImplementedRequirement AssemblyOscalSspImplementedRequirement `json:"implemented-requirement,omitempty"`
}

// AssertOscalImplementedRequirementRequired checks if the required fields are not zero-ed
func AssertOscalImplementedRequirementRequired(obj OscalImplementedRequirement) error {
	if err := AssertAssemblyOscalSspImplementedRequirementRequired(obj.ImplementedRequirement); err != nil {
		return err
	}
	return nil
}

// AssertRecurseOscalImplementedRequirementRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of OscalImplementedRequirement (e.g. [][]OscalImplementedRequirement), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseOscalImplementedRequirementRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aOscalImplementedRequirement, ok := obj.(OscalImplementedRequirement)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertOscalImplementedRequirementRequired(aOscalImplementedRequirement)
	})
}
