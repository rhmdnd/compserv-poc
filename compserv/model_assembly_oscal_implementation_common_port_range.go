...
// AssemblyOscalImplementationCommonPortRange - Where applicable this is the IPv4 port range on which the service operates.
type AssemblyOscalImplementationCommonPortRange struct {
...
// AssertAssemblyOscalImplementationCommonPortRangeRequired checks if the required fields are not zero-ed
func AssertAssemblyOscalImplementationCommonPortRangeRequired(obj AssemblyOscalImplementationCommonPortRange) error {
	return nil
}

// AssertRecurseAssemblyOscalImplementationCommonPortRangeRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of AssemblyOscalImplementationCommonPortRange (e.g. [][]AssemblyOscalImplementationCommonPortRange), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseAssemblyOscalImplementationCommonPortRangeRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aAssemblyOscalImplementationCommonPortRange, ok := obj.(AssemblyOscalImplementationCommonPortRange)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertAssemblyOscalImplementationCommonPortRangeRequired(aAssemblyOscalImplementationCommonPortRange)
	})
}
